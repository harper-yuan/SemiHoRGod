...
OnlineEvaluator::OnlineEvaluator(int id,  //复制创建评估器
                                 std::shared_ptr<io::NetIOMP<5>> network,
...
void OnlineEvaluator::setInputs(
  ...
  std::vector<size_t> num_inp_pid(5, 0);
  ...
  for (int recv_pid = 0; recv_pid < 5; ++recv_pid) {
  ...
  std::vector<size_t> pid_inp_idx(5, 0);
...
std::vector<BoolRing> BoolEvaluator::reconstruct(
  ...
  std::array<std::vector<uint8_t>, 4> packed_recon_shares;
  for (int i = 0; i < 4; ++i) {
...
void BoolEvaluator::evaluateGatesAtDepth(size_t depth, io::NetIOMP<5>& network,
                                         ImprovedJmp& jump,
                                         ThreadPool& tpool) {
...
void BoolEvaluator::evaluateAllLevels(io::NetIOMP<5>& network,
...
std::vector<std::vector<BoolRing>> BoolEvaluator::getOutputShares(io::NetIOMP<5>& network,
                                      ImprovedJmp& jump, ThreadPool& tpool) {
